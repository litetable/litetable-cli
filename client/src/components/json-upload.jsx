...
export default function JsonUpload({ handleSubmit }) {
	...
	const validateJson = (jsonData) => {
		...
		for (let i = 0; i < jsonData.length; i++) {
			...
			if (!item.rowkey) return { valid: false, message: `Item ${i+1} is missing rowkey` };
			if (!item.family) return { valid: false, message: `Item ${i+1} is missing family` };
			if (!item.qualifiers || typeof item.qualifiers !== 'object') {
				return { valid: false, message: `Item ${i+1} has invalid qualifiers` };
			}
		}
		...
		try {
			...
			for (let i = 0; i < dataArray.length; i++) {
				...
				const payload = {
					...
					key: item.rowkey,
					family: item.family,
					qualifiers: Object.entries(item.qualifiers).map(([qualifier, value]) => ({
						name: qualifier,
				...
				<Tabs defaultValue="upload" className="w-full">
					...
					<TabsContent value="upload" className="space-y-4">
						<div className="grid w-full max-w-sm items-center gap-1.5">
							<label htmlFor="json-file" className="cursor-pointer border-2 border-dashed rounded-md p-6 text-center hover:bg-muted/50">
					...
					{jsonContent && (
						<div className="mt-4 space-y-4">
							...
							<div className="bg-muted rounded-md p-3 max-h-[200px] overflow-y-auto">
								<pre className="text-xs">{jsonContent.slice(0, 500)}{jsonContent.length > 500 ? '...' : ''}</pre>
							</div>
						</div>
					)}
					...
					{uploadStatus === "success" && (
						<Alert className="mt-4 bg-green-50 border-green-200">
							...
							<AlertDescription>All {totalCount} records were processed successfully.</AlertDescription>
						</Alert>
					)}
